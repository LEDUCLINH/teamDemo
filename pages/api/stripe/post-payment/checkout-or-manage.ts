    ...
    try {
      ...
      if (isNational) {
        const session = await stripe.billingPortal.sessions.create({
          ...
          return_url: `${process.env['NEXT_PUBLIC_BASE_URL']}/post/${post.id}`,
        })
        url = session.url
      } else {
        const session = await stripe.checkout.sessions.create({
          ...
          mode: 'subscription',
          success_url: `${process.env['NEXT_PUBLIC_BASE_URL']}/post/${post.id}`,
          cancel_url: `${process.env['NEXT_PUBLIC_BASE_URL']}/post/${post.id}`,
          metadata: {
            user_id: String(user.id),
            post_id: String(post.id),
            updated_post_is_local: String(updated_post_is_local),
          },
          customer: post.stripe_customer_id || undefined,
        })
        url = session.url
      }
